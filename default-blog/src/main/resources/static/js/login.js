$(function () {
    ...
    var imageCode_null = $("#imageCode_null");
    loginFormSubmit.click(function () {
        var password_len = password.val().length;
        var phone_len = phone.val().length;
        var phone_value = phone.val();
        var imageCode_len = imageCode.val().length;
        var imageCode_value = imageCode.val();
        var pattren1 = /^1[345789]\d{9}$/;

        if(password_len !== 0 && phone_len !== 0 && pattren1.test(phone_value) && imageCode_len !== 0){
            // 这里可以判断验证码
            var check = false;
            $.ajax({
                type:'get',
    ...
    change_password_btn.click(function () {
        ...
        if(change_password_btn.hasClass("yes_submit") && change_password_btn.hasClass("password_right") && modal_phone.hasClass("phone_success")){
            $.ajax({
                ...
                success:function (data) {
                    if(data['status'] === 902 || data['status'] === 1203){
    ...
    var loginSmsFormSubmit = $("#loginSmsFormSubmit");
    var mobile = $("#mobile");
    var sms_phone_null = $("#sms_phone_null");
    var sms_phone_error = $("#sms_phone_error");
    var smsCode = $("#smsCode");
    var smsCode_null = $("#smsCode_null");
    var sms_login_error = $(".sms_login_error")
    mobile.addClass("phone_error");
    getSmsCode.addClass("not_send");

    // 输入手机号框失去焦点
    mobile.blur(function () {
        var mobile_len = mobile.val().length;
        var mobile_value = mobile.val();
        if(mobile_len === 0){
            sms_phone_null.show();
            mobile.removeClass("phone_success");
            mobile.addClass("phone_error");
        }
        if(mobile_len !== 0){
            var pattren1 = /^1[345789]\d{9}$/;
            if(!pattren1.test(mobile_value)){
                sms_phone_error.show();
                mobile.removeClass("phone_success");
                mobile.addClass("phone_error");
            }
            if(pattren1.test(mobile_value)){
                mobile.removeClass("phone_error");
                mobile.addClass("phone_success");
            }
        }
    });
    // 输入手机号框获得焦点
    mobile.focus(function () {
        sms_phone_error.hide();
        sms_phone_null.hide();
        sms_login_error.css("visibility","hidden");
    });

    // 定义发送时间间隔(s)
    var sms_interval;
    sms_interval = 60;
    var sms_timeLeft = sms_interval;
    //重新发送计时函数
    var sms_timeCount = function() {
        window.setTimeout(function() {
            if(sms_timeLeft > 0) {
                sms_timeLeft -= 1;
                getSmsCode.html(sms_timeLeft + "秒重新发送");
                sms_timeCount();
            } else {
                getSmsCode.html("重新发送");
                sms_timeLeft = 60;
    ...
    getSmsCode.click(function () {
        ...
        if(mobile.hasClass("phone_success")){
            $.ajax({
                type:'get',
                url:'/code/sms',
                dataType:'json',
                ...
                success:function (data) {
                    if(parseInt(data['status']) === 0) {
                        getSmsCode.removeClass("not_send");
                        getSmsCode.addClass("has_send");
                        alert("短信验证码发送成功");
                        sms_timeCount();
                    } else {
                        alert("短信验证码发送异常");
    ...
    loginSmsFormSubmit.click(function () {
        var smsCode_len = smsCode.val().length;
        var mobile_len = mobile.val().length;
        var mobile_value = mobile.val();
        var pattren1 = /^1[345789]\d{9}$/;
        if (smsCode_len !== 0 && mobile_len !== 0 && pattren1.test(mobile_value) && mobile.hasClass("phone_success") && getSmsCode.hasClass("has_send")) {
            return true;
        } else {
            if (getSmsCode.hasClass("not_send")) {
                alert("手机验证码还未发送！！！")
            }
            if(mobile_len === 0){
                sms_phone_null.show();
            }
            if(!pattren1.test(mobile_value) && mobile_len !== 0){
                sms_phone_error.show();
            }
            if (smsCode_len === 0) {
                smsCode_null.show();
