...
function putInArticle(data) {
    ...
    var articleFooter = $('<div class="end-logo">' +
        ...
        '<li><strong>本文原作者：</strong><span id="authorFooter">' + data.originalAuthor + '</span></li>' +
    ...
    // 收藏按钮
    var favoriteBtn = $('<div class="favoriteBtn am-btn am-btn-secondary">' +
        '<div class="favoriteHeart">' +
        '<i class="am-icon-star-o">&nbsp;&nbsp;收藏</i>' +
        '</div>' +
        '<div class="favoritesNum">' +
        '<span> ' + data.favorites + '</span>' +
        '</div>' +
        '</div>');
    other.append(favoriteBtn);
    if (data.isFavorite === 1) {
        ...
        $('.favoriteHeart').find('i').removeClass("am-icon-star-o");
        $('.favoriteHeart').find('i').addClass("am-icon-star");
    }
    ...
    $("#wordsView img").each(function (index) {
        if (!$(this).hasClass("emoji")) {
            var a = $(this).attr('src');
            var aTop = a.replace(/oss.czodly.top/g, "cwz-blog.oss-cn-shanghai.aliyuncs.com");
            $(this).attr("data-src", aTop);
...
function putInComment(data) {
    ...
    if (data.length === 0) {
        ...
        var newComment = $('<div class="new-comment">' +
            '<i class="all-comment am-icon-ellipsis-v"></i>全部评论（<span class="commentNum">' + data[data.length - 1] + '</span>）' +
            '</div>');
        ...
        $.each(data, function (index, obj) {
            ...
            if (index !== data.length - 1) {
                ...
                subComment.append($('<div class="reply-sub-comment-list am-animation-slide-bottom">' +
                    ...
                    '<button type="button" class="quitReplyWordBtn am-btn am-round am-btn-default">取消</button>' +
                    '</div>' +
                    '</div>' +
                    '</div>'));
                ...
                var visitorSay = $('<div class="visitorSay">' + obj['commentContent'] + '</div>');
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data['status'] === 101) {
                        ...
                        var orgin_user = $this.parent().parent().parent().parent().prev().prev().prev().find('.visitorName').html();
                        //alert(orgin_user);
                        ...
                        var oldcommentNum = commentNum.html();
                        commentNum.html(parseInt(oldcommentNum) + 1);
        ...
        success: function (data) {
            if (data['status'] === 0) {
                ...
                var error = $('<div class="article"><div class="zhy-article-top"><div class="error">' +
                    '<img src="https://oss.czodly.top/blog/image/article/common/no1.jpg">' +
                    '<p>没有找到这篇文章哦~~~</p>' +
                    '<p>该篇文章可能被博主或者管理员删除了 (┬＿┬)</p>' +
                    '<div class="row">' +
                    '<a href="/">返回首页</a>' +
                    '</div>' +
                    '</div></div></div>');
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data['status'] === 101) {
                    ...
                    alert("该篇文章您已经收藏过了！ (*^__^*) ");
                } else {
                    $('.favoritesNum').find('span').html(data['data']);
                    $('.favoriteBtn').css({
                        "background-color": "#56b6e7",
                        "color":"white"
                    });
